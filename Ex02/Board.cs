...
namespace Ex02
{
    internal class Board
    {
        ...
        public Board(State i_GameState)
        {
        ...
        public void DisplayBoard()
        {
            ...
            Console.WriteLine("| # # # # |        |");
            Console.WriteLine("|==================|");
            for (int i = 0; i < r_GameState.MaxNumberOfGuesses; i++)
            {
                if (i < r_GameState.m_UserGuesses.Count)
                {
                    string currentGuess = string.Join(" ", r_GameState.m_UserGuesses[i].ToCharArray()).PadRight(7);
                    string currentResult = string.Join(" ", r_GameState.m_Results[i].ToCharArray()).PadRight(6);
